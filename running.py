...
schedule = Scheduler()
schedule.daily(dtime.time(hour=9, minute=45), header)  
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    ...
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        area1 = [(550,150),(w-0,150),(w-0,700),(110,700)]
                        area2 = [(0,780),(w-0,780),(w-0,1080),(0,1080)]
                        ...
                        if label=='Plat':
                            if result2 >= 0:
                                if id not in data_p:
                                    ...
                                    # ret, img_binary = cv2.threshold(sharp_img, 100, 255, cv2.THRESH_BINARY_INV)
                                    # cv2.imwrite('D:/Program Python/Yolo Terbaru/plat-detection/crop1.jpg',img_binary)
                                    # # dilasi = cv2.dilate(img_binary,None,iterations=1)
                                    # kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3,3))
                                    # opening = cv2.morphologyEx(img_binary, MORPH_OPEN,kernel)
                                    cv2.imwrite('D:/Program Python/Yolo Terbaru/plat-detection/crop.jpg',noise)
                                    # text1 = image_to_string(dilasi)
                                    # karnel = np.ones((1,1),np.uint8)
                                    # dlt = cv2.dilate(crop,karnel,iterations=1)
                                    # ero = cv2.erode(dlt,karnel,iterations=1)
                                    # gray = cv2.cvtColor(ero, cv2.COLOR_BGR2GRAY)
                                    # cv2.imwrite('D:/Program Python/Yolo Terbaru/plat-detection/crop.jpg',gray)
                                    # (thresh, blackAndWhiteImage) = cv2.threshold(gray, 130, 255, cv2.THRESH_BINARY_INV)
                                    # cv2.imwrite('D:/Program Python/Yolo Terbaru/plat-detection/crop1.jpg',blackAndWhiteImage)                           
                                    text1 = pytesseract.image_to_string(noise, config='-l eng --oem 3 --psm 7 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789')
                                    annotator.box_label(bboxes, text1, color=colors(c, True))
                                    data_p.append(id)
                                if id in data_p:
                                    annotator.box_label(bboxes, text1, color=colors(c, True))
                        # if result2 >= 0:
            ...
            if show_vid:
                ...
                cv2.putText(im0, str(t_truksedang) + ' Truk Sedang', (1050,140), font, 
                    fontScale, color_font, thickness, cv2.LINE_AA)
                frame = cv2.resize(im0,(1280,720),fx=0,fy=0, interpolation = cv2.INTER_CUBIC)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--yolo_model', nargs='+', type=str, default='dataset/database.pt', help='model.pt path(s)')
    parser.add_argument('--deep_sort_model', type=str, default='osnet_x0_25')
    parser.add_argument('--source', type=str, default='videos/IMG_0690.MOV', help='source')  # file/folder, 0 for webcam
